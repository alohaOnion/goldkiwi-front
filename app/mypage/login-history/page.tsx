...
import { useLoginHistory } from "@/lib/hooks/use-profile";
...
export default function LoginHistoryPage() {
  ...
  const { data: loginHistory, isLoading, error } = useLoginHistory(true, {
    take: PAGE_SIZE + 1,
      ...
      <header className="sticky top-0 z-50 w-full border-b border-zinc-800/50 bg-zinc-950/80 backdrop-blur-2xl">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">로그인 이력</h1>
              <p className="text-xs text-zinc-400">IP와 로그인 시간</p>
            </div>
          </div>
        </div>
      </header>
          ...
          <CardContent className="pt-6">
            {isLoading ? (
              ...
              <div className="py-16 text-center">
                <LogIn className="h-16 w-16 text-zinc-600 mx-auto mb-4" />
                <p className="text-zinc-500">아직 로그인 이력이 없습니다.</p>
